        ...
        <div className='py-10 min-h-[80vh]'>
          ...
          <div className='flex flex-wrap gap-3.5'>
            {projects.map((project)=>(
              <div onClick={()=>navigate(`/projects/${project.id}`)} key={project.id} className='relative group w-72 max-sm:mx-auto cursor-pointer bg-gray-900/60 border border-gray-700 round-lg overflow-hidden shadow-md group hover:shadow-indigo-700/30 hover:border-indigo-800/80 transition-all duration-300'>
                {/*desktop like mini preview */}
                <div className='relative w-full h-40 bg-gray-900 overflow-hidden border-b border-gray-800 rounded-lg
                shadow-md group hover:shadow-amber-700/30 hover:border-indigo-800/80 transition-all duration-300'>
                ...
                :(
                  <div className='flex flex-items justify-center h-full text-gray-500'>
                    <p>No Preview</p>
                  </div>
                )}
                </div>
                ...
                <div onClick={e=>e.stopPropagation()}>
                  <TrashIcon className='absolute top-3 right-3 sacale-0 group-hover:scale-100 bg-white p-1.5 size-7 rounded text-red-500 text-xl cursor-pointer transition-all' onClick={()=>deleteProject(project.id)}/>
                </div>
              </div>
            ))}
          </div>
