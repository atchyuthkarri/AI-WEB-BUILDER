...
const Projects = () => {
    ...
    <div className='flex flex-col h-screen w-full bg-gray-900 text-white'>
      ...
      <div className='flex max-sm:flex-col sm:items-center gap-4 px-4 py-2 no-scrollbar'>
        ...
        <div className='flex items-center justify-end gap-3 flex-1 text-xs sm:text-sm'>
          ...
          <Link target="_blank" className='flex items-center gap-2 px-4 py-1 rounded sm:rounded sm:rounded-sm border border-gray-700 hover:border-gray-500 transition-colors' to={`/preview/${projectId}`}>
          ...
          <button className='bg-linear-to-br from-blue-700 to-blue-600 hover:from-blue-600 hover:to-blue-500 text-white px-3.5 py-1 flex items-center gap-2 rounded sm:rounded-sm transition-colors'>
            ...
            {project.isPublished ? "Unpublish":"Published"}
          </button>
        </div>
      </div>
    </div>
  ...
  (
    <div className='=flex items-center justify-center h-screen'>
      <p className='text-2xl font-medium text-gray-200'>Oh-Oh! Unable to load Projects</p>
    </div>
  )
}
